...
int execute_flushdb_command(zval* object, int argc, zval* return_value, zend_class_entry* ce) {
    ...
    if (execute_core_command(
            valkey_glide, &core_args, NULL, process_core_bool_result_batch, return_value)) {
        if (valkey_glide->is_in_batch_mode) {
            /* In batch mode, return $this for method chaining */
            ZVAL_COPY(return_value, object);
            return 1;
...
int execute_pfadd_command(zval* object, int argc, zval* return_value, zend_class_entry* ce) {
    ...
    long result;
    if (execute_core_command(
            valkey_glide, &args, &result, process_core_bool_result_batch, return_value)) {
        if (valkey_glide->is_in_batch_mode) {
            /* In batch mode, return $this for method chaining */
            ZVAL_COPY(return_value, object);
            return 1;
...
int execute_select_command_internal(valkey_glide_object* valkey_glide, long dbindex) {
    ...
    return execute_core_command(
        valkey_glide, &args, NULL, process_core_bool_result_batch, &dummy_return);
}
...
int execute_move_command_internal(valkey_glide_object* valkey_glide,
    ...
    return execute_core_command(
        valkey_glide, &args, output_value, process_core_bool_result_batch, &dummy_return);
}
