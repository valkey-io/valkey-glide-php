...
/* Execute a CLIENT CACHING command using the Valkey Glide client */
int execute_client_caching_command(zval*             object,
    ...
    /* Parse parameters: enabled (required bool) */
    if (zend_parse_method_parameters(argc, object, "Ob", &object, ce, &enabled) == FAILURE) {
        return 0;
    }
    ...
    core_command_args_t core_args = {0};
    core_args.glide_client        = valkey_glide->glide_client;
    core_args.cmd_type            = ClientCaching;
    core_args.is_cluster          = (ce == get_valkey_glide_cluster_ce());

    /* Add YES or NO argument */
    core_args.args[0].type                  = CORE_ARG_TYPE_STRING;
    core_args.args[0].data.string_arg.value = enabled ? "YES" : "NO";
    core_args.args[0].data.string_arg.len   = enabled ? 3 : 2;
    core_args.arg_count                     = 1;
...
/* Execute a CLIENT TRACKING command using the Valkey Glide client */
int execute_client_tracking_command(zval*             object,
    ...
    /* Parse parameters: on (required bool), options (optional array) */
    if (zend_parse_method_parameters(argc, object, "Ob|a!", &object, ce, &on, &z_options) ==
        FAILURE) {
        return 0;
    }
    ...
    core_command_args_t core_args = {0};
    core_args.glide_client        = valkey_glide->glide_client;
    core_args.cmd_type            = ClientTracking;
    core_args.is_cluster          = (ce == get_valkey_glide_cluster_ce());

    /* First arg: ON or OFF */
    core_args.args[0].type                  = CORE_ARG_TYPE_STRING;
    core_args.args[0].data.string_arg.value = on ? "ON" : "OFF";
    core_args.args[0].data.string_arg.len   = on ? 2 : 3;
    core_args.arg_count                     = 1;

    /* Dynamic arg list for complex options */
    core_arg_t* all_args    = NULL;
    int         total_args  = 1; /* Start with ON/OFF */
    int         max_args    = CORE_ARGS_FIXED_MAX;
    int         use_dynamic = 0;
    ...
    if (z_options && Z_TYPE_P(z_options) == IS_ARRAY) {
        ...
        /* Count maximum possible args: ON/OFF + REDIRECT id + PREFIX p1 p2... + BCAST + OPTIN +
         * OPTOUT + NOLOOP */
        int potential_args = 1; /* ON/OFF */
        ZEND_HASH_FOREACH_STR_KEY_VAL(ht, key, val) {
            if (key) {
                if (zend_string_equals_literal_ci(key, "redirect")) {
                    ...
                    potential_args += 1; /* BCAST, OPTIN, OPTOUT, NOLOOP */
                }
            }
        }
        ZEND_HASH_FOREACH_END();

        if (potential_args > CORE_ARGS_FIXED_MAX) {
            all_args    = (core_arg_t*) ecalloc(potential_args, sizeof(core_arg_t));
            use_dynamic = 1;
            max_args    = potential_args;
            /* Copy first arg */
            all_args[0].type                  = CORE_ARG_TYPE_STRING;
            all_args[0].data.string_arg.value = on ? "ON" : "OFF";
            all_args[0].data.string_arg.len   = on ? 2 : 3;
        }
        ...
        ZEND_HASH_FOREACH_STR_KEY_VAL(ht, key, val) {
            ...
            if (zend_string_equals_literal_ci(key, "redirect")) {
                /* REDIRECT <client-id> */
                if (total_args + 2 > max_args)
                    break;
                core_arg_t* args_arr      = use_dynamic ? all_args : core_args.args;
                args_arr[total_args].type = CORE_ARG_TYPE_STRING;
                args_arr[total_args].data.string_arg.value = "REDIRECT";
                args_arr[total_args].data.string_arg.len   = 8;
                total_args++;
                ...
                if (Z_TYPE_P(val) == IS_ARRAY) {
                    zval* prefix_val;
                    ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(val), prefix_val) {
                        if (total_args + 2 > max_args)
                            break;
                        if (Z_TYPE_P(prefix_val) != IS_STRING)
                            continue;
                        args_arr[total_args].type                  = CORE_ARG_TYPE_STRING;
                        args_arr[total_args].data.string_arg.value = "PREFIX";
                        args_arr[total_args].data.string_arg.len   = 6;
                        total_args++;
                        args_arr[total_args].type                  = CORE_ARG_TYPE_STRING;
                        args_arr[total_args].data.string_arg.value = Z_STRVAL_P(prefix_val);
                        args_arr[total_args].data.string_arg.len   = Z_STRLEN_P(prefix_val);
                        total_args++;
                    }
                    ZEND_HASH_FOREACH_END();
                } else if (Z_TYPE_P(val) == IS_STRING) {
                    if (total_args + 2 <= max_args) {
                        ...
                        total_args++;
                        args_arr[total_args].type                  = CORE_ARG_TYPE_STRING;
                        args_arr[total_args].data.string_arg.value = Z_STRVAL_P(val);
                        args_arr[total_args].data.string_arg.len   = Z_STRLEN_P(val);
                        total_args++;
                    }
                }
            } else if (zend_string_equals_literal_ci(key, "bcast")) {
                if (zval_is_true(val) && total_args < max_args) {
                    core_arg_t* args_arr      = use_dynamic ? all_args : core_args.args;
                    args_arr[total_args].type = CORE_ARG_TYPE_STRING;
                    args_arr[total_args].data.string_arg.value = "BCAST";
                    args_arr[total_args].data.string_arg.len   = 5;
                    total_args++;
                }
            } else if (zend_string_equals_literal_ci(key, "optin")) {
                if (zval_is_true(val) && total_args < max_args) {
                    core_arg_t* args_arr      = use_dynamic ? all_args : core_args.args;
                    args_arr[total_args].type = CORE_ARG_TYPE_STRING;
                    args_arr[total_args].data.string_arg.value = "OPTIN";
                    args_arr[total_args].data.string_arg.len   = 5;
                    total_args++;
                }
            } else if (zend_string_equals_literal_ci(key, "optout")) {
                if (zval_is_true(val) && total_args < max_args) {
                    core_arg_t* args_arr      = use_dynamic ? all_args : core_args.args;
                    args_arr[total_args].type = CORE_ARG_TYPE_STRING;
                    args_arr[total_args].data.string_arg.value = "OPTOUT";
                    args_arr[total_args].data.string_arg.len   = 6;
                    total_args++;
                }
            } else if (zend_string_equals_literal_ci(key, "noloop")) {
                if (zval_is_true(val) && total_args < max_args) {
                    core_arg_t* args_arr      = use_dynamic ? all_args : core_args.args;
                    args_arr[total_args].type = CORE_ARG_TYPE_STRING;
                    args_arr[total_args].data.string_arg.value = "NOLOOP";
                    args_arr[total_args].data.string_arg.len   = 6;
                    total_args++;
                }
            }
    ...
    core_args.arg_count = total_args;

    if (use_dynamic) {
        core_args.all_args = all_args;
    }
