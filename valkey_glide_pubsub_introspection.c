...
void valkey_glide_pubsub_impl(INTERNAL_FUNCTION_PARAMETERS, const void* connection) {
    ...
    if (strcasecmp(cmd, "channels") == 0) {
        ...
        struct CommandResult* result = command(connection, 0, (enum RequestType)COMMAND_REQUEST__REQUEST_TYPE__PubSubChannels, argc, args, args_len, NULL, 0, 0);
        if (args) efree(args);
        ...
        struct CommandResult* result = command(connection, 0, (enum RequestType)COMMAND_REQUEST__REQUEST_TYPE__PubSubNumSub, argc, args, args_len, NULL, 0, 0);
        efree(args);
        efree(args_len);
        ...
        // PUBSUB NUMPAT
        struct CommandResult* result = command(connection, 0, (enum RequestType)COMMAND_REQUEST__REQUEST_TYPE__PubSubNumPat, 0, NULL, NULL, NULL, 0, 0);
