...
if test "$PHP_VALKEY_GLIDE" != "no"; then
  ...
  dnl Add FFI library only to linker flags, not compiler flags
  PHP_ADD_LIBRARY_WITH_PATH(glide_ffi, valkey-glide/ffi/target/release, VALKEY_GLIDE_SHARED_LIBADD)
  
  dnl Add resolv library - platform specific
  case $host_os in
    darwin*)
      PHP_ADD_LIBRARY(resolv, 1, VALKEY_GLIDE_SHARED_LIBADD)
      ;;
    linux*)
  ...
  VALKEY_GLIDE_SHARED_LIBADD="$VALKEY_GLIDE_SHARED_LIBADD \$(abs_builddir)/valkey-glide/ffi/target/release/libglide_ffi.a"
