...
int valkey_glide_otel_init(zval* config_array) {
    if (g_otel_initialized) {
        VALKEY_LOG_WARN("otel_init", "OpenTelemetry already initialized, ignoring subsequent calls");
...
int parse_otel_config_array(zval* config_array, valkey_glide_otel_config_t* otel_config) {
    ...
    if (traces_val && Z_TYPE_P(traces_val) == IS_ARRAY) {
        ...
        if (endpoint_val && Z_TYPE_P(endpoint_val) == IS_STRING) {
            ...
            VALKEY_LOG_ERROR("otel_config", "Traces endpoint is required when traces config is provided");
            return 0;
        }
        
        /* Parse sample_percentage with default of 1% (like Java/Go) */
        zval* sample_val = zend_hash_str_find(traces_ht, "sample_percentage", sizeof("sample_percentage") - 1);
    ...
    if (metrics_val && Z_TYPE_P(metrics_val) == IS_ARRAY) {
        ...
        if (endpoint_val && Z_TYPE_P(endpoint_val) == IS_STRING) {
            ...
            VALKEY_LOG_ERROR("otel_config", "Metrics endpoint is required when metrics config is provided");
            return 0;
        }
